...
BEGIN
  PERFORM pg_advisory_xact_lock(a_id_cliente);
	SELECT * INTO cliente_data FROM transacoes WHERE id_cliente = a_id_cliente order by id desc limit 1;
  ...
  IF copy_valor < 0 AND cliente_data.saldo + copy_valor < cliente_data.limite * -1 THEN
    SELECT -2, -2, -2 INTO result;
  ELSE
      INSERT INTO transacoes (saldo, limite, valor, descricao, tipo, realizada_em, id_cliente)
        VALUES (cliente_data.saldo + copy_valor, cliente_data.limite, valor, descricao, tipo, NOW(), a_id_cliente)
